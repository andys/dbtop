...
import (
	...
	"github.com/andys/dbtop/internal/db"
...
func (a *App) refresh(ctx context.Context) {
	...
	for _, p := range procs {
		if a.dbType == "MySQL" {
			// MySQL: filter out Sleep commands
			if p.State != "Sleep" && p.State != "" {
				active = append(active, p)
			}
		} else {
			// PostgreSQL: filter out idle state
...
func (a *App) render() {
	...
	if a.dbType == "MySQL" {
		line3 = fmt.Sprintf(
			"Commits: %d  Rollbacks: %d  DiskReads: %d  BufHits: %d  RowsRead: %d  RowsInserted: %d",
			a.stats.XactCommit,
...
func (a *App) showDetail() {
	...
	// EXPLAIN
	sb.WriteString("\n── EXPLAIN ──\n\n")
	explainOutput, err := a.db.Explain(ctx, proc.Query)
	if err != nil {
	...
	// Locks
	sb.WriteString("\n── Locks ──\n\n")
	locks, err := a.db.GetLocks(ctx, proc.PID)
	if err != nil {
